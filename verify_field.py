...
def query_ollama_api(prompt: str, pil_image_path: str, model: str) -> Optional[str]:
    ...
    print(f"Debug - Raw Model Response: {raw_response}", file=sys.stderr)

    if not raw_response or not raw_response.strip():
        print("Debug - 'response' field is empty, checking 'thinking'...", file=sys.stderr)
        # If 'response' is empty, try 'thinking' field (common for some Ollama models)
        raw_response = full_ollama_response.get("thinking", "")
        if raw_response:
             print(f"Debug - Found content in 'thinking': {raw_response[:100]}...", file=sys.stderr)
        else:
             print(f"Debug - Full Response Keys: {list(full_ollama_response.keys())}", file=sys.stderr)
